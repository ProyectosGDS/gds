    ...
    <div v-if="props.fields.length > 0" ref="target" class="w-full relative">
        <div class="flex items-center gap-1">
            <input
                ...
                class="peer m-0 block bg-white w-full rounded border focus-within:border-violet-400 bg-transparent bg-clip-padding px-3 py-3.5 text-base font-normal leading-tight text-neutral-700 transition duration-200 ease-linear placeholder:text-transparent focus:border-primary focus:pb-[0.625rem] focus:pt-[1.625rem] focus:text-neutral-700 focus:outline-none peer-focus:text-primary [&:not(:placeholder-shown)]:pb-[0.625rem] [&:not(:placeholder-shown)]:pt-[1.625rem]"
            ...
            <label
                class="pointer-events-none absolute left-0 top-0 origin-[0_0] border border-solid border-transparent px-3 py-3.5 text-violet-400 transition-[opacity,_transform] duration-200 ease-linear peer-focus:-translate-y-2 peer-focus:translate-x-[0.15rem] peer-focus:scale-[0.85] peer-focus:text-gray-400 peer-[:not(:placeholder-shown)]:-translate-y-2 peer-[:not(:placeholder-shown)]:translate-x-[0.15rem] peer-[:not(:placeholder-shown)]:scale-[0.85] motion-reduce:transition-none uppercase">
                {{ props.title }}
            </label>
        </div>
        <div :class="`absolute bg-white border p-4 mt-1 rounded border-violet-200 z-10 ${dropdownPosition === 'top' ? 'bottom-full mb-1' : 'top-full mt-1'}`" v-show="open">
            ...
            <ul class="h-auto max-h-48 overflow-auto">
                <template v-for="(item, index) in data">
                    <li v-if="!item.deleted_at" @click="selected(item)" ref="listRefs" class=" whitespace-nowrap"
