...
<template>
    ...
    <Card class="bg-white p-8">
        <h1 class="text-2xl text-gray-500 font-semibold">Escuelas</h1>
        <div class="flex justify-center">
            <Tool-Tip message="Crear nueva escuela" class="-mt-8 text-violet-400">
                <Button @click="store.openModalNew = true" icon="fas fa-plus" class="btn-primary" />
            </Tool-Tip>
        </div>
        <Data-Table :headers="store.headers" :data="store.escuelas" :loading="store.loading" :export="false">
    ...
    <Modal :open="store.openModalNew" icon="fas fa-school" title="Crear nueva escuela" class="w-96">
        ...
        <div>
            <Input v-model="store.escuela.nombre" option="label" title="Nombre de la escuela" :error="store.errors.hasOwnProperty('nombre')" />
            <Input v-model="store.escuela.direccion" option="text-area" title="Dirección completa" :error="store.errors.hasOwnProperty('direccion')" />
            <Input v-model="store.escuela.descripcion" option="text-area" title="Descripción" />
        ...
        <div>
            <div class="flex justify-end">
                <div class="flex gap-3 text-gray-500">
                    <span>Activo</span>
                    <Switch :values="[null,0]" v-model="store.escuela.deleted_at" class="bg-gray-400 w-12 has-[:checked]:bg-violet-400"/>
                    <span>Inactivo</span>
                </div>
            </div>
            <br>
            <Input v-model="store.escuela.nombre" option="label" title="Nombre de la escuela" :error="store.errors.hasOwnProperty('nombre')" :disabled="store.escuela.deleted_at" />
            <Input v-model="store.escuela.direccion" option="text-area" title="Dirección completa" :error="store.errors.hasOwnProperty('direccion')" :disabled="store.escuela.deleted_at" />
            <Input v-model="store.escuela.descripcion" option="text-area" title="Descripción" />
            <Validate-Errors :errors="store.errors" v-if="store.errors != 0" />
        </div>
        <template #footer>
    ...
    <Modal :open="store.openModalDelete">
        <div class="flex items-center gap-4">
            ...
            <div class="grid">
                <h1>¿Esta seguro de eliminar la escuela?</h1>
                <h2 class="text-center font-semibold text-xl">{{ store.escuela.nombre }}</h2>
            </div>
        </div>
        <template #footer>
            <Button @click="store.resetData" text="Cancelar" icon="fas fa-xmark" class="btn-dark rounded-full" />
