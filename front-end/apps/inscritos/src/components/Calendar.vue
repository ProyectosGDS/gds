...
<div v-if="props.min">
    ...
    <div ref="calendarContainer" class="min-h-full min-w-full text-gray-800">
        <div class="w-full border grid grid-cols-7">
            ...
            <div v-for="day in daysInCurrentMonth" :key="day" class="h-14 w-full border align-top">
                <div class="w-full h-full text-xs md:text-sm lg:text-base text-center transition-colors" 
                    ...
                    <div v-if="allTodaysEvent(day, events).length > 0" class="flex w-full justify-center items-center cursor-pointer" @click="openModal(day, allTodaysEvent(day, events))">
                        <div class="h-6 w-6 flex justify-center items-center text-xs bg-violet-600 rounded-full shadow-sm">
...
<div v-else>
    ...
    <Modal :open="openModalEvent" :title="evento.date?.start" icon="fas fa-calendar-days" class="w-96">
        <template #header>
            {{ evento.title }}
        </template>
        <div v-if="evento.hasOwnProperty('date')" class="grid gap-6">
            <Input type="text" icon="fas fa-bell" v-model="evento.title" disabled />
            <Input type="date" icon="fas fa-calendar-days" v-model="evento.date.start" disabled />
            <Input type="time" icon="fas fa-clock" v-model="evento.time.start" disabled />
            <textarea v-model="evento.description" rows="4" class="input h-28 p-4" disabled>
            </textarea>
        </div>
        <template #footer>
            <Button @click="openModalEvent = false" text="cerrar" class="btn-primary rounded-full" />
        </template>
    </Modal>

    <Modal v-if="allTodayEvent.hasOwnProperty('allEvents')" :open="openModalAllEvent" :title="allTodayEvent.allEvents[0].date.start" icon="fas fa-calendar-days" class="w-96">
                ...
                <summary class="cursor-pointer text-violet-500 font-medium">
                    ...
                    <Transition name="fade">
                        <div class="grid gap-2 p-2">
                            <span>
                                {{ `${event.date.start} ${event.time.start} -- a -- ${event.date.end} ${event.time.end} ` }}
                            </span>
                            <span>
                                {{ event.description }}
                            </span>
        ...
        <template #footer>
            <Button @click="resetData" text="Cerrar" class="btn-primary rounded-full" /> 
        </template>
    </Modal>
</template>