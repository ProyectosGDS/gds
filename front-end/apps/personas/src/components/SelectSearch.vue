    ...
    const data = computed(() => {
        if(props.sort) {
            ...
            return props.items.filter(item => 
                item[props.fields[1]].toLowerCase().includes(search.value.toLocaleLowerCase())
            ).sort((a,b) => a[props.fields[1]].localeCompare(b[props.fields[1]]))
...
<template>
    <div ref="target" class="w-full">
        <div class="flex items-center space-x-2 input bg-white" :class="{'border-red-500' : props.error }" >
            <input v-bind="$attrs"  :value="props.format ? displayValue : item[props.fields[1]]" readonly @click="openDropdown" @focus="openDropdown" class="focus:outline-none w-full h-12 placeholder:text-violet-400 cursor-pointer" />
