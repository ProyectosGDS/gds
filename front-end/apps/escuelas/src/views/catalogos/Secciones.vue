...
<template>
    ...
    <Card class="bg-white p-8">
        <h1 class="text-2xl text-gray-500 font-semibold">Secciones</h1>
        ...
        <Data-Table :headers="store.headers" :data="store.secciones" :loading="store.loading" :export="false">
    ...
    <!-- AREA DE MODALES -->
    <Modal :open="store.openModalNew" icon="fas fa-school" title="Crear nueva seccion" class="w-96">
        <template #close>
            <Icon @click="store.resetData" icon="fas fa-xmark" class="text-violet-200 text-xl hover:scale-125 cursor-pointer" />
        </template>

        <div class="grid gap-3">
            <Input v-model="store.seccion.nombre" option="label" title="Nombre del seccion" :error="store.errors.hasOwnProperty('nombre')" />
    ...
    <Modal :open="store.openModalEdit" title="Editar seccion" icon="fas fa-school" class="w-96">
        ...
        <div class="grid gap-3">
            <div v-if="store.seccion.deleted_at" class="flex justify-end">
                <div class="flex gap-3 text-gray-500">
                    <span>Activo</span>
                    <Switch :values="[null,0]" v-model="store.seccion.deleted_at" class="bg-gray-400 w-12 has-[:checked]:bg-violet-400"/>
                    <span>Inactivo</span>
                </div>
            </div>
            <br>
            <Input v-model="store.seccion.nombre" option="label" title="Nombre del seccion" :error="store.errors.hasOwnProperty('nombre')" :disabled="store.seccion.deleted_at" />
            <Validate-Errors :errors="store.errors" v-if="store.errors != 0" />
        </div>
        <template #footer>
            <Button @click="store.resetData" text="Cancelar" icon="fas fa-xmark" class="btn-dark rounded-full" />
    ...
    <Modal :open="store.openModalDelete">
        <div class="flex items-center gap-4">
            ...
            <div class="grid">
                ...
                <h2 class="text-center font-semibold text-xl">{{ store.seccion.nombre }}</h2>
            </div>
        </div>
        <template #footer>
            <Button @click="store.resetData" text="Cancelar" icon="fas fa-xmark" class="btn-dark rounded-full" />
