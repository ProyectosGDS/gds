<script setup>
    ...
    const escuelas = computed(() => {
        return localStorage.getItem('escuelas') === 'true' ? true : false
    })
...
<template>
    ...
    <Modal :open="store.openModalNew" icon="fas fa-school" title="Crear nueva escuela" class="w-96">
        ...
        <div class="grid gap-3">
            ...
            <Input v-if="escuelas" v-model="store.programa.escuela_id" option="select" title="Seleccione escuela" :error="store.errors.hasOwnProperty('escuela_id')">
                <option value=""></option>
    ...
    <Modal :open="store.openModalEdit" title="Editar programa" icon="fas fa-school" class="w-96">
        ...
        <div class="grid gap-3">
            <div v-if="store.programa.deleted_at" class="flex justify-end">
            ...
            <Input v-if="escuelas" v-model="store.programa.escuela_id" option="select" title="Seleccione escuela" :error="store.errors.hasOwnProperty('escuela_id')" :disabled="store.programa.deleted_at">
                <option value=""></option>
                <template v-for="escuela in escuelaStore.escuelas">
                    <option v-if="!escuela.deleted_at" :value="escuela.id">{{ escuela.nombre }}</option>
                </template>
